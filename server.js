...
//Procesamos nueva comprobación en la nube - TEST
// ==========================================
//  MÓDULO: REVISIÓN DE DATOS EN LA NUBE (CORREGIDO FINAL)
// ==========================================
app.post('/api/revision-nube', isAuthenticated, async (req, res) => {
    ...
    // Empresa (02=Cocineria, 04=Mar Picante, 06=Abruzzo)
    if (empresa === '02') {
        idEmpresa = 2; idCajero = 2; prefixTicket = 'T001';
    } else if (empresa === '04') {
        idEmpresa = 4; idCajero = 4; prefixTicket = 'T002';
    } else if (empresa === '06') {
        idEmpresa = 6; idCajero = 6; prefixTicket = 'T005';
    } else {
        return res.status(400).json({ message: 'Empresa no válida' });
    }
    ...
    try {
        ...
        // --- CONSULTA 1: DOCCAB ---
        // Usamos CAST(Fecha AS DATE) para ignorar la hora y tomar todo el día
        const qDoccab = await pool.request()
            .input('emp', sql.Int, idEmpresa)
            .input('turno', sql.Int, idTurno)
            .input('f1', sql.VarChar, fechaInicio)
            .input('f2', sql.VarChar, fechaFin)
            .query(`
                SELECT MIN(Numero) as First, MAX(Numero) as Last, COUNT(*) as Total 
                FROM Doccab 
                WHERE Empresa = @emp AND Turno = @turno 
                AND Eliminado = 0
                AND CAST(Fecha AS DATE) BETWEEN @f1 AND @f2
            `);

        // --- CONSULTA 2: DOCDET ---
        const qDocdet = await pool.request()
            .input('emp', sql.Int, idEmpresa)
            .input('turno', sql.Int, idTurno)
            .input('f1', sql.VarChar, fechaInicio)
            .input('f2', sql.VarChar, fechaFin)
        ...
        // --- CONSULTA 3: CAJA ---
        const qCaja = await pool.request()
            .input('cajero', sql.Int, idCajero)
            .input('tipoCaja', sql.Int, idTurno)
            .input('f1', sql.VarChar, fechaInicio)
            .input('f2', sql.VarChar, fechaFin)
            .query(`
                SELECT MIN(Numero) as First, MAX(Numero) as Last, COUNT(*) as Total 
        ...
        // --- CONSULTA 4: TICKET_C (MAESTRO) ---
        // AHORA AGREGAMOS EL FILTRO DE TURNO TAMBIÉN
        // Y usamos CAST para asegurar que tome todo el día
        const qTicketC = await pool.request()
            .input('prefix', sql.VarChar, prefixTicket + '%')
            .input('turno', sql.Int, idTurno)
            .input('f1', sql.VarChar, fechaInicio)
            .input('f2', sql.VarChar, fechaFin)
            .query(`
                SELECT MIN(NroTicket) as First, MAX(NroTicket) as Last, COUNT(*) as Total 
                FROM Ticket_c 
                WHERE NroTicket LIKE @prefix 
                AND Turno = @turno
                AND CAST(Fecha AS DATE) BETWEEN @f1 AND @f2
            `);
        ...
        // Si hay tickets maestros, buscamos sus detalles y pagos
        if (rangeTickets.Total > 0) {

            const minTicket = rangeTickets.First;
            const maxTicket = rangeTickets.Last;

            // --- CONSULTA 5: TICKET_D ---
            qTicketD = await pool.request()
                .input('minT', sql.VarChar, minTicket)
                .input('maxT', sql.VarChar, maxTicket)
                .query(`
                    SELECT MIN(NroTicket) as First, MAX(NroTicket) as Last, COUNT(*) as Total 
                    FROM Ticket_d 
                    WHERE NroTicket >= @minT AND NroTicket <= @maxT
                `);

            // --- CONSULTA 6: PAGOS_TICKETS ---
            qPagos = await pool.request()
                .input('minT', sql.VarChar, minTicket)
                .input('maxT', sql.VarChar, maxTicket)
                .query(`
                    SELECT MIN(NroTicket) as First, MAX(NroTicket) as Last, COUNT(*) as Total 
        ...
        res.json({
            doccab: qDoccab.recordset[0],
            docdet: qDocdet.recordset[0],
            caja: qCaja.recordset[0],
            ticket_c: rangeTickets,
            ticket_d: qTicketD.recordset[0],
            pagos: qPagos.recordset[0]
        });

    } catch (error) {
        console.error("Error en revisión nube:", error);
        res.status(500).json({ message: 'Error consultando base de datos' });
    }
});
